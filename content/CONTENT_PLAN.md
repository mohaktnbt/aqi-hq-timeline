# AQI HQ Content Plan — Phase 2: WordPress Content Hub

> **Author:** Article Writer Agent (61d43add-86e4-4c49-aac7-be17197d396e)
> **Date:** 2026-03-17
> **Status:** In Progress

---

## Overview

This document outlines the content strategy for the AQI HQ WordPress Content Hub, Phase 2 of the AQI HQ operational mandate.

### Objective

Establish a high-authority WordPress blog dedicated to air quality health impacts and education to:
1. Build topical authority around air quality, health, indoor air, and purifiers
2. Drive organic traffic back to the main AQI dashboard
3. Support future e-commerce monetization (Phase 3)

### Target Metrics

- Domain authority score growth
- Backlink profile strength
- Organic sessions from search
- Content engagement (time on page, bounce rate)

---

## Content Pillars

### Pillar 1: AQI Basics & Education (Priority 1)

**Goal:** Establish foundational authority on air quality concepts

| Article Topic | Target Keywords | Status |
|---------------|-----------------|--------|
| What is AQI? Understanding Air Quality Index | "what is aqi", "air quality index meaning", "aqi full form" | Planned |
| How to Read AQI: Color Codes & Health Implications | "aqi color codes", "how to read air quality index", "aqi health levels" | Planned |
| PM2.5 vs PM10: Understanding Particulate Matter | "pm2.5 vs pm10", "particulate matter health effects", "difference between pm2.5 and pm10" | Planned |
| Real-time AQI Monitoring: Why It Matters for Indian Cities | "real time aqi india", "live air quality monitor", "city wise aqi today" | Planned |
| Indian Air Quality Standards: CPCB Guidelines Explained | "cpcb aqi standards", "indian air quality standards", "national ambient air quality standards" | Planned |

### Pillar 2: Health & Wellness (Priority 2)

**Goal:** Connect air quality to health outcomes and solutions

| Article Topic | Target Keywords | Status |
|---------------|-----------------|--------|
| Health Effects of Poor Air Quality: Respiratory & Cardiovascular | "health effects of air pollution", "pollution health risks", "air quality health impacts" | Planned |
| Protecting Children from Air Pollution: A Parent's Guide | "protect children from air pollution", "kids air quality health", "child pollution safety" | Planned |
| Indoor Air Quality: Why It Matters More Than You Think | "indoor air quality importance", "home air pollution", "indoor aqi levels" | Planned |
| Air Purifiers: How They Work and When You Need One | "how do air purifiers work", "when to use air purifier", "air purifier benefits" | Planned |
| Breathing Exercises for Pollution Days | "breathing exercises for pollution", "yoga for air pollution", "clean air breathing" | Planned |

### Pillar 3: City-Specific Guides (Priority 1)

**Goal:** Capture hyper-local search traffic for major Indian cities

| City | Target Keywords | Status |
|------|-----------------|--------|
| Delhi | "delhi aqi today", "delhi air quality index", "pollution level in delhi" | Planned |
| Mumbai | "mumbai aqi today", "mumbai air quality index", "pollution level in mumbai" | Planned |
| Bangalore | "bangalore aqi today", "bengaluru air quality", "pollution level in bangalore" | Planned |
| Chennai | "chennai aqi today", "chennai air quality index", "pollution level in chennai" | Planned |
| Kolkata | "kolkata aqi today", "kolkata air quality index", "pollution level in kolkata" | Planned |
| Hyderabad | "hyderabad aqi today", "hyderabad air quality index", "pollution level in hyderabad" | Planned |

### Pillar 4: Seasonal & Event-Based Content (Priority 3)

**Goal:** Capture trending searches during pollution seasons

| Topic | Timing | Status |
|-------|--------|--------|
| Winter Pollution in North India: Causes & Solutions | November - February | Planned |
| Diwali Air Pollution: Health Risks & Precautions | October - November | Planned |
| Summer Dust Storms: Impact on Air Quality | March - May | Planned |
| Monsoon Season: How Rain Affects AQI | June - September | Planned |

---

## Content Templates

### Standard Article Template

```
# [H1: Article Title]

[Meta Description: 150-160 characters]

## Introduction
[200-300 words: Hook, context, what reader will learn]

## [H2: Section 1]
[400-600 words]

### [H3: Subsection 1.1]
[200-300 words]

### [H3: Subsection 1.2]
[200-300 words]

## [H2: Section 2]
[400-600 words]

### [H3: Subsection 2.1]
[200-300 words]

### [H3: Subsection 2.2]
[200-300 words]

## [H2: Section 3]
[400-600 words]

## [H2: Practical Tips/Action Steps]
[300-400 words]

## [H2: FAQ Section]
[300-400 words: 5-7 questions with answers]

## Conclusion
[150-200 words: Summary, call to action]

---

**Word Count:** [1500-2500 words]
**Primary Keyword:** [Keyword]
**Secondary Keywords:** [Keywords]
**Meta Title:** [60 characters max]
**Meta Description:** [150-160 characters]
```

### City Guide Template

```
# [H1: Air Quality in [City Name]: A Complete Guide]

[Meta Description: 150-160 characters including city name]

## Current AQI Status in [City]
[Real-time data section]

## Understanding [City]'s Air Quality Challenges
[Local pollution sources, seasonal patterns]

## Health Impacts for [City] Residents
[Local health statistics, vulnerable populations]

## Best Practices for [City] Residents
[City-specific recommendations]

## Monitoring Air Quality in [City]
[Local monitoring stations, apps, resources]

## FAQ: [City] Air Quality
[City-specific questions]

---

**Word Count:** [1500-2000 words]
**Primary Keyword:** "[city] aqi today"
```

---

## Production Schedule

### Week 1 (March 17-23)
- [ ] Article 1: "What is AQI? Understanding Air Quality Index"
- [ ] Article 2: "How to Read AQI: Color Codes & Health Implications"
- [ ] Content template finalization

### Week 2 (March 24-30)
- [ ] Article 3: "PM2.5 vs PM10: Understanding Particulate Matter"
- [ ] Article 4: "Delhi Air Quality: Complete Guide"
- [ ] Begin city-specific content series

### Week 3 (March 31 - April 6)
- [ ] Article 5: "Health Effects of Poor Air Quality"
- [ ] Article 6: "Mumbai Air Quality: Complete Guide"
- [ ] Internal linking strategy implementation

### Week 4 (April 7-13)
- [ ] Article 7: "Protecting Children from Air Pollution"
- [ ] Article 8: "Bangalore Air Quality: Complete Guide"
- [ ] SEO optimization audit

---

## SEO Guidelines

### On-Page SEO Checklist
- [ ] Primary keyword in H1
- [ ] Primary keyword in first 100 words
- [ ] Secondary keywords in H2s
- [ ] Meta title under 60 characters
- [ ] Meta description 150-160 characters
- [ ] URL slug includes primary keyword
- [ ] Image alt text with keywords
- [ ] Internal links to other articles
- [ ] External links to authoritative sources
- [ ] FAQ schema markup
- [ ] Article schema markup

### Content Quality Standards
- [ ] Minimum 1500 words (2500 for pillar articles)
- [ ] Fact-checked information with sources
- [ ] No spelling or grammar errors
- [ ] Proper heading hierarchy (H1 > H2 > H3)
- [ ] Readable sentence length (avg 15-20 words)
- [ ] Active voice preferred
- [ ] Mobile-responsive formatting
- [ ] Clear call-to-action

---

## Success Metrics

### Short-term (30 days)
- 5 articles published
- 10,000+ words of content created
- 50+ internal links established
- Basic schema markup implemented

### Medium-term (90 days)
- 15+ articles published
- 50,000+ words of content
- 500+ backlinks acquired
- Domain authority increase

### Long-term (6 months)
- 30+ articles published
- 100,000+ words of content
- 2,000+ backlinks
- Top 3 rankings for target keywords

---

## Next Actions

1. ✅ Create content directory structure
2. ✅ Draft content plan
3. ⏳ Write Article 1: "What is AQI? Understanding Air Quality Index"
4. ⏳ Create content calendar for next 4 weeks
5. ⏳ Set up content workflow and templates

---

**Document Version:** 1.0
**Last Updated:** 2026-03-17
**Next Review:** 2026-03-24
